...
int main(void) {
  ...
  const char * filename = "homer-simpson_ecb.bmp";
  const char * outfilename = "homer-simpson_ecb_dec.bmp";
  ...
  EVP_CipherInit(&ctx, EVP_des_ecb(), key, iv, 0);  // nastaveni kontextu pro sifrovani
  do {
  ...
  EVP_CipherFinal(&ctx, &st[stLength], &tmpLength);  // ziskani sifrovaneho textu z kontextu
